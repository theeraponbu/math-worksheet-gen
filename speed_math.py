...
class MathProPDF(FPDF):
    ...
    def generate_sheet(self, problems, title, school, teacher, f_size, f_style, col_gap, row_gap, 
        ...
        for p_idx in range(0, len(problems), probs_per_page):
            ...
            # Header Section (Fixed Area: 0-45mm)
            ...
            # --- BOX LOGIC: ล็อกเนื้อหาในกล่อง ---
            col_width = (self.w - 30) / 4 + (col_gap / 15)
            # ล็อกความสูงกล่องโจทย์แต่ละข้อให้คงที่ (35mm) เพื่อป้องกันเฉลยทับกัน
            box_height = 30 + (row_gap / 10) 
            
            for i, p in enumerate(page_probs):
                col_i, row_i = i % 4, i // 4
                x, y = 15 + col_i * col_width, 55 + row_i * box_height
                
                num = (i + 1) if restart_num else (p_idx + i + 1)
                
                # วาดข้อความภายในกล่อง (Absolute Coordinates)
                self.set_xy(x, y)
                ...
                # เฉลย (Answer) - ล็อกพิกัดใต้เส้นคั่น
                if is_key:
                    self.set_text_color(220, 0, 0)
                    self.set_xy(x+5, y+22); self.cell(20, 10, f"{p['a']+p['b']:>3}", align='R')
...
def run_app():
    ...
    with st.sidebar:
        ...
        num_pages = st.selectbox("Generate Pages", [1, 10])
    ...
    # --- PERFECT PREVIEW WITH BOTTOM AREA ---
    ...
    items_html = "".join([f"""
        <div style="width: 22%; height: {25 + row_gap/5}px; margin-bottom: 20px; font-family: {font_css}; color: black; font-size: {f_size}px; position: relative; border: 0.5px dashed transparent;">
    ...
    # ปรับปรุงพื้นที่แสดงผลให้เห็นขอบกระดาษครบถ้วน
    components.html(f"""
        <div style="background: #e0e0e0; display: flex; flex-direction: column; align-items: center; padding: 100px 0; min-height: 1200px; overflow-y: auto;">
            <div style="width: 700px; height: {700 * aspect}px; background: white; padding: 50px; box-shadow: 0 10px 40px rgba(0,0,0,0.4); box-sizing: border-box; flex-shrink: 0; position: relative;">
