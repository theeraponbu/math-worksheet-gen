...
# --- 1. PDF ENGINE (STRICT MM COORDINATES) ---
class MathProPDF(FPDF):
    ...
    def generate_sheet(self, problems, title, school, teacher, f_size, f_style, col_gap, row_gap, 
        ...
        for p_idx in range(0, len(problems), probs_per_page):
            ...
            # --- Grid Area: Start 55mm, End 250mm ---
            col_w = (self.w - 30) / 4 + (col_gap / 20)
            # กระจายระยะบรรทัดให้พอดีกับความสูงกระดาษที่เหลือ
            rows_on_page = (probs_per_page // 4)
            available_h = self.h - 90 # เว้นขอบล่าง 
            row_h = (available_h / rows_on_page) + (row_gap / 10)
            ...
            for i, p in enumerate(page_probs):
                ...
                # เส้นคั่นใต้โจทย์ (Clean Short Line)
...
# --- 2. PPTX & MATH LOGIC --- (ส่วนนี้ใช้ Logic เดิมที่เสถียรแล้ว)
...
def run_app():
    ...
    # --- PERFECT SYNC PREVIEW (WITH TOP/BOTTOM GRAY SPACE) ---
    ...
    # คอนเทนเนอร์สีเทาที่โอบล้อมทั้งบนและล่าง
    components.html(f"""
        <div style="background: #ccc; display: flex; flex-direction: column; align-items: center; padding: 100px 0; min-height: 1200px; overflow-y: auto;">
            <div style="width: 700px; height: {700 * aspect}px; background: white; padding: 50px; box-shadow: 0 15px 50px rgba(0,0,0,0.5); box-sizing: border-box; flex-shrink: 0; margin-bottom: 100px;">
                <div style="text-align: center; border-bottom: 2px solid #000; margin-bottom: 20px; color: black; font-family: sans-serif;">
                    ...
                    <h2 style="margin: 5px 0; font-size: 26px;">{title}</h2>
    ...
    with col_pptx:
        ...
        st.download_button("📊 Download PPTX", data=ppt_data, file_name=f"{title}.pptx", use_container_width=True)
