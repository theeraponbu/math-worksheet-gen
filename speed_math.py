...
class GlobalMathPDF(FPDF):
    ...
    def draw_page(self, title, school, teacher, problems, font_size, scale):
        ...
        self.set_line_width(0.5) # ปรับเส้นคั่นหัวข้อให้หนาขึ้น
        ...
        self.set_line_width(0.6) # ปรับเส้นคำตอบให้หนาคมชัด
        for idx, p in enumerate(problems):
            ...
            # วาดเส้นคำตอบให้พอดีกับช่วงตัวเลข
...
def run_app():
    ...
    for i, p in enumerate(st.session_state.current_math):
        # แก้ไข CSS ให้เส้นสั้นลงและตัวหนาขึ้น
        item = (
            f'<div style="text-align:right; font-size:{font_size}px; font-family:Arial, sans-serif; font-weight:bold; width:80px; margin:auto;">'
            f'<span style="float:left; font-size:12px; color:gray; font-weight:normal;">{i+1})</span>'
            f'{p["a"]}<br>+{p["b"]}'
            f'<div style="border-bottom:3px solid black; width:60px; margin-left:auto; margin-top:2px;"></div>'
            f'</div>'
        )
    ...
    <div style="width:{preview_width}px; height:{preview_width * aspect_ratio}px; background:white; margin:auto; border:1px solid #ddd; box-shadow:0 4px 15px rgba(0,0,0,0.1); padding:40px; color:black; overflow:hidden;">
        <div style="text-align:center; border-bottom:2px solid black; padding-bottom:10px; margin-bottom:20px;">
            ...
            <div style="font-weight:bold; font-size:28px; margin:10px 0; font-family:Arial;">{ws_title}</div>
        ...
        <div style="display:grid; grid-template-columns:repeat(4, 1fr); gap:30px; transform:scale({content_scale/100}); transform-origin:top center; padding-top:20px;">
            {all_problems_html}
        </div>
    </div>
    ...
    try:
        ...
        st.download_button(label="📥 Download Professional PDF", data=pdf_bytes, file_name=f"{ws_title}.pdf", mime="application/pdf", key="dl_final")
