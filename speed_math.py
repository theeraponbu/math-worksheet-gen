...
class GlobalMathPDF(FPDF):
    ...
    def draw_page(self, title, school, teacher, problems, font_size, scale):
        ...
        self.set_line_width(0.6) # เส้นคำตอบหนาเท่าหน้าจอ
        for idx, p in enumerate(problems):
            ...
            # วาดเส้นคำตอบให้สั้นลงพอดีกับตัวเลข (60mm scale down)
...
def run_app():
    ...
    # --- 3. THE MASTER PREVIEW (ZERO ERROR HTML) ---
    st.subheader("📄 Live Designer Preview")
    aspect_ratio = 1.29 if paper_size == "Letter" else 1.41
    preview_width = 720 
    
    # สร้างโจทย์ด้วยวิธีประกอบ String ที่ปลอดภัยที่สุด
    all_problems_html = ""
    for i, p in enumerate(st.session_state.current_math):
        all_problems_html += (
            f'<div style="text-align:right; font-size:{font_size}px; font-family:Arial, sans-serif; font-weight:bold; width:80px; margin:auto; margin-bottom:20px;">'
    ...
    # ใช้ unsafe_allow_html ครอบคลุมชุดข้อมูลเดียวทั้งหมดเพื่อป้องกัน Error ติ่งโค้ด
    canvas_container = f"""
    <div style="width:{preview_width}px; height:{preview_width * aspect_ratio}px; background:white; margin:auto; border:1px solid #ddd; box-shadow:0 4px 15px rgba(0,0,0,0.1); padding:50px; color:black; overflow:hidden;">
        <div style="text-align:center; border-bottom:2px solid black; padding-bottom:10px; margin-bottom:25px;">
            ...
            <div style="font-weight:bold; font-size:32px; margin:15px 0; font-family:Arial;">{ws_title}</div>
    ...
    try:
        ...
        st.download_button(label="📥 Download Professional PDF", data=pdf_bytes, file_name=f"{ws_title}.pdf", mime="application/pdf", key="dl_final_master")
